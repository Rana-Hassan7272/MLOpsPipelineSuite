...
def load_model_from_s3(model_key: str) -> Optional[Any]:
    ...
    try:
        ...
        if not s3_bucket:
            ...
            print(f"  S3_BUCKET env not set, skipping S3 load for {model_key}")
        ...
        with tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as tmp:
            ...
            s3.download_fileobj(s3_bucket, f"models/{model_key}.pkl", tmp)
        ...
        print(f"  ✓ Loaded {model_key} from s3://{s3_bucket}/models/{model_key}.pkl")

        return model, "s3-latest"

    except Exception as e:

        print(f"  ⚠ Could not load {model_key} from S3: {e}")
...
def load_models():
    ...
    # Load Logistic Regression - try S3 first, then MLflow registry
    try:

        result = load_model_from_s3("logistic_regression")
        ...
        if result[0] is not None:
            ...
            print(f"✓ Loaded logistic_regression (version: {result[1]})")

        else:

            print("⚠ logistic_regression not found")

    except Exception as e:

        print(f"⚠ Could not load logistic_regression: {e}")

    

    # Load K-Means - try S3 first, then MLflow registry
    try:

        result = load_model_from_s3("kmeans")
        ...
        if result[0] is not None:
            ...
            print(f"✓ Loaded kmeans (version: {result[1]})")
    ...
    # Load Isolation Forest - try S3 first, then MLflow registry
    try:

        result = load_model_from_s3("isolation_forest")
        ...
        if result[0] is not None:
            ...
            print(f"✓ Loaded isolation_forest (version: {result[1]})")

        else:

            print("⚠ isolation_forest not found")

    except Exception as e:

        print(f"⚠ Could not load isolation_forest: {e}")
